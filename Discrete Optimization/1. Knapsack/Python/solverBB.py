...
#			print "Rama eliminada con profundidad " + str(n.depth)
			elim = n.branch
			i = 1
			while (elim[-i]==0):
				del pa[2][-1]
...
# Comprobamos si al añadir el siguiente se nos va por lo que acabamos la 
	elif ((n.depth == pa[3])or((n.depth == pa[3]-1)and(n.room - pa[0][pa[3]-1] < 0))):
		if (n.value > pa[4]):
			print "New Best:" + str (n.value)
			pa[4] = n.value
			pa[5] = n.branch + (pa[3] - n.depth)*[0]
...
#			print "Rama acabada"
			...
			i = 1
			del pa[2][-1]
# Caso de que sea el ultimo eliminamos demasiados
			while ((elim[-i -1]==0)):
				del pa[2][-1]
				i = 1+i
				if (i >= len(n.branch)):
...
def solveIt(inputData):
    ...
    for i in range(0, items):
	average += density[i][0]
    average = (average / items)*capacity

...
import copy
# For copying an array of nodes because we cannot use [:] with clases
class node:
...
#		print "Node: " + str(self.number) + " added node " + str(edge)
	def set_colors (self, num_colors):
		self.posible = range (0, num_colors)
...
# If there is any pruning we put in a queue the pruned nodes to check if we can givi them a color
...
#		print "Color " + str(color)+" a remover de: " + str( conected_node.number)

		if (conected_node.posible.count(color) == 1 ):
			conected_node.posible.remove(color)
		if (ok == 0):
			if (len (conected_node.posible)== 1):
				nodes_queue.insert(-1, conected_node.number) # We add it to the queue por pruning
	#We can only put one in the queue coz if one is in the queue and pruning another one makes it infeasible
...
#	print " Removiendo nodo: " + str(node_to_color) + " con color " + str(color) + " Conec: " + str(nodes[order[node_to_color]].edges)
	for i in range(0, nodes[order[node_to_color]].n_edges):	# For every conection the node had
		...
		nodes[order[conected_node]].edges.remove (node_to_color)	# We remove the conection from the nodes
...
#	print "Decision is node " + str (ordered_list[0].number)
	nodes_queue.insert(-1,ordered_list[len(ordered_list)-1].number)
...
# Now we add to the branches to the nodes
    ...
    nodes_copy = copy.deepcopy(nodes) # We copy the nodes so that if the initial number of nodes
				# is not fasible we can start over
    nodes_aux = nodes[:]

# Now we check for the minimum number of colors it can have, which is the maximum number
# of conections between the connected nodes to a node for CP

#    print "ColoreS: " + str(n_colors[0])
...
# Remove color
	colored = []
	nodes = copy.deepcopy(nodes_copy)
	queue = []
	n_colors[0] -= 1
	order =  range(0,nodeCount)
	nodes_queue = []
	...
	#			print " Nodo " + str(nodes[i].number) + " --> " +str(nodes[i].edges)
	#			print " Nodo " + str(nodes_copy[i].number) + " --> " +str(nodes_copy[i].edges)
			decision_maker(nodes, nodes_queue)		# We start the decion maker which will select a node toç
			times_em += 1
	#		print "Decision " + str(times_em)

	#	print "Cola:"
	#	print nodes_queue

		feasible = color_node (nodes,nodes_queue[-1],aux_conections, colored, nodes_queue, order); 

	# We color the node, which will propagate and if it was unfeasible (needed more color than what already had), increase colors.

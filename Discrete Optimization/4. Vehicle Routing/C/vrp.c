...
int main() {
    ...
    float aux_dis;		/* Aux variable for a greedy algorith */

    int i,j,k,l,r, cont, aux_car, costum,c, aux_j;			/*Contadores */
...
/* Simply: 
	We will try diferent starting feasible points, do SA in every trayectory and thats it */
   srand(1513);
/*-------------------------------------------------------------------------------------------------------------*/
for (r = 0; r < 30000; r++ ) {
...
permutate_order(order, n_costum-1);	/* Reorder */
tak = 0;


/* Gredy now: We will use different aproaches */
/*
...
*/
// For every costumer 
	for (i = 0; i < n_cars; i++ ){		// For every car we asing a costumer to them
		car_costumers[i][n_costum_car[i]] = order[i]+1;
		cap_left[i] -= C[order[i]+1];
		n_costum_car[i] += 1;
		tak += 1;
	}

   for ( i = i; i < n_costum -1; i++) {		// For every costumer left
	aux_dis = 1000000;
	aux_j = -1;
	for (j = 0; j < n_cars; j++ ){
		if (cap_left[j] - C[order[i]+1] >= 0 ) {	// IF it fits
			if (length (order[i]+1,car_costumers[j][n_costum_car[j]-1], X,Y) < aux_dis) {
				aux_dis = length (order[i]+1,car_costumers[j][n_costum_car[j]-1], X,Y);
				aux_j = j;
			}
		}
	}
...
//		printf("No se puede hacer \n");
	...
	car_costumers[aux_j][n_costum_car[aux_j]] = order[i]+1;
	cap_left[aux_j] -= C[order[i]+1];
	n_costum_car[aux_j] += 1;
	tak += 1;

    }


   if (tak ==  n_costum - 1){/* If we could fit all costumers */
...
printf("------------------------\n");
	   ...
	   for (i = 0; i < total_cars; i++ ){
		pos = findint(best_solutions[i], n_costum_car[i], 0);	
		for (j = pos; j < n_costum_car[i]; j++ ) {
			final_best[i][j - pos] = best_solutions[i][j];
		}
		for (j = 0; j <= pos; j++ ){
			final_best[i][ n_costum_car[i] - pos + j] = best_solutions[i][j];	
		}
	    }
	}
}
     ...
     print_vrp ( final_best, n_costum_best, total_cars , big_best, "out");

...
def best_swap (capas, layer, position, solution, points, possible_points):
...
#------------------------------------------------------------------------------------------------------------------------#
def temple_simulado(T,capas, layer, position, solution, points, possible_points,path, best, best_points):

# best_will[0] be the best implementation ever got
# new_best will be the points of the best implementation so far
# T : Temperatura
# capas y layer las posiciones del nodo a cambiar
# position: 
...
# Get the ending nodes of the 2 vertex we are changing
		...
		delta = ini - fin			# The new path fin has to be shorter 
#		print T
		prob = math.exp(-abs(delta)/T)
		if ((delta >= 0) or ( prob > randi)) and abs(pos1 - pos2) > 1:
			final = i
			pos2 = solution.index (possible_points[final])
			OTP2(pos1,pos2,solution)
			path[0] = path[0] - delta
...
#	print capas_fin[i]
    ...
    get_possible(possible_points, capas_fin, 5)
...
# Print the original solution !!!--------------------------------------------------------------------------------------
    ...
    best = [get_path (points, solution)]
    path = [get_path (points, solution)]
    print best[0]
    best_points = points[:]

    T = 10.0
    kon = 0.9999
    # We do this till we are stuck.
...
#		print "Layer: " + str(i)
		for j in range (0,len(capas_fin[i])):
			...
			temple_simulado (T,capas_fin, i , j, solution, points, possible_points[i][j],path, best, best_points)
	print get_path (points, solution)
    print "Temple simulado hecho"
    print ""
    print "Mejor " + str(best)
    solution = best_points
    print solution
...
#		print "Layer: " + str(i)
		for j in range (0,len(capas_fin[i])):
			best_swap (capas_fin, i , j, solution, points, possible_points[i][j])	 # First and last
	print get_path (points, solution)

    print "Ultima mejora hecha OTP"
    print ""
    print "Mejor " + str(best)

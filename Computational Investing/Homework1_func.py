...
def get_Portfolio_metrics_CovMatrix(Returns, allocation):
  # Get the returns
    
    # Obtain the metrics for every symbol
    Cum_Ret = np.sum(Returns,0)
    E_Return = np.mean(Returns,0)
    ...
    P_E_return = E_Return.dot(allocation)
    P_Cum_Ret = Cum_Ret.dot(allocation)
    
    # Matrix form Rist
    P_std_Return = np.sqrt(allocation.dot(Cov_Ret).dot(allocation))
    
    P_SharpR = P_E_return/P_std_Return
    P_SortinoR = SortinoR.dot(allocation)  # This is actually wrong
...
def plot_efficient_frontier(start_date, end_date, symbols, keys, Nrandom, Covariance = 1):
    # This function gets several data from your portfolio
    d_data, timestamps = get_Prices (start_date, end_date, symbols, keys)
    Returns = get_Symbol_returns (d_data, 'close', symbols)
    ...
    for i in range (Nrandom):
        if (Covariance):
            ...
            P_E_return, P_Cum_Ret,P_std_Return ,P_SharpR ,P_SortinoR = get_Portfolio_metrics_CovMatrix(Returns, Allocations[i,:])
...
def plot_covariance (start_date, end_date, symbols, keys, Nrandom, influence = 1):
    # This function gets several data from your portfolio
    d_data, timestamps = get_Prices (start_date, end_date, symbols, keys)
    Returns = get_Symbol_returns (d_data, 'close', symbols)
    ...
    # Normalize Returns
    Returns = Returns - np.mean(Returns,0)
    Returns = Returns /np.std(Returns,0)
    
    # Covariance
    Cov_Ret = np.cov(Returns.T)
    Corr_Ret = np.corrcoef(Returns.T)
    # Correlation is invariant of normalization of data 

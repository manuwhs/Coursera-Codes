...
class CFGraph():
    def __init__(self, V):  #create an empty graph with V vertices
    ...
    def addEdge(self,v1,v2,w):  # Adds an edge between edges v1 and v2
        edge = CEdge(v1,v2,w)
        ### CAREFUL !! We add the same edge in both places, but it is the same edge.
        ## In directed Graphs it was inverted.
